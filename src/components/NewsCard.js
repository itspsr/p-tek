...
export default function NewsCard({ article, minimal = false }) {
  ...
  return (
    <Link href={`/articles/${article.id}`} className="group glass-card block overflow-hidden">
      <div className={`relative ${minimal ? "h-32" : "h-48"} w-full overflow-hidden`}>
        ...
        <div className="absolute bottom-3 left-3">
          <span className="text-xs font-mono text-ptek-blue uppercase bg-black/50 px-2 py-1 rounded border border-ptek-blue/20">
            {article.category}
          </span>
        </div>
      </div>
      ...
      <div className="p-4 space-y-3">
        ...
        <h3 className={`font-semibold text-gray-100 group-hover:text-ptek-blue transition ${minimal ? "text-sm" : "text-lg"}`}>
          {article.title}
        </h3>
